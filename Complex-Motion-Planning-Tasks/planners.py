...
class RRT_STAR(object):
    ...
    def find_path(self, start_conf, goal_conf):
        ...
        found_goal=False
        #######
        # TODO - Nir - check if  "conf_new_id==-1" is required?
        while i < self.max_itr or not found_goal:

            if i % self.reset_threshold == 0:  # Check if it's time to reset
                print(f"Resetting search at iteration {i}...")
                self.reset_search()  # Reset the search
                start_id = self.add_vertex(start_conf)  # Re-add the start vertex after reset
                i =1
            ...
            if self.bb.local_planner(conf_near, conf_new):
                ...
                if np.array_equal(conf_new, goal_conf):
                    found_goal=True
                    print("RRT* num of interation to find the goal: ",i)
                    if (self.goal_id == -1):
    ...
    def rewire(self, x_potential_parent_id, x_child_id) -> None:
        ...
        if self.is_edge_new(x_child_id, x_potential_parent_id):
            if self.bb.local_planner(self.tree.vertices[x_potential_parent_id], self.tree.vertices[x_child_id]) :
                cost_new_edge = self.bb.edge_cost(self.tree.vertices[x_potential_parent_id], 
                                                  self.tree.vertices[x_child_id])
