...
class Visualize_Gif(object):
    def __init__(self, ur_params, env, transform, bb, waypoints_coords):
        ...
        self.waypoints_coords = waypoints_coords
    ...
    def draw_bead_maze(self):
        ...
        self.ax.scatter(self.waypoints_coords[:, 0], 
                        self.waypoints_coords[:, 1], 
                        self.waypoints_coords[:, 2], color='b', s=5)
    ...
    def save_path_to_gif(self, path_list: list[list[list[float]]], filename: str = None):
        """
        Inputs:
        ...
        for config in path_list:
            ...
            self.ax.set_zlim3d(0, 2)
        if filename:
            imageio.mimsave(filename + '.gif', images, fps=10)
        else:
