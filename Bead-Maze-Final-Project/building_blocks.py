...
class Building_Blocks(object):
    ...
    def is_in_collision(self, conf) -> bool:
        ...
        # arm - bead maze collision
        obstacles = self.env.bead_maze_obstacle
        if obstacles.size > 0:
            for joint in self.ur_params.ur_links:
                # if joint == 'wrist_1_link' or joint == 'wrist_2_link' or joint == 'wrist_3_link':
    ...
    def local_planner(self, prev_conf, current_conf, splines):
        ...
        for config in intermediate_configs:
            ...
            if current_dist > 0.03:#TODO - why 10?
                return True, np.inf
